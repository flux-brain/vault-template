...
2. Weekly check: run `date -u +%u%H`. If it prints `7<weekly-hour>` (Sunday, the last hourly run of the day) and `briefings/weekly/$(TZ=<TZ> date +%G-W%V).md` does not exist yet, you will ALSO do the weekly project review and the lint (CLAUDE.md Operations 3 and 4) after step 3.
...
Note: CLAUDE.md's run protocol (the `.run/active` marker, "answers first", the filed summary) applies on top of this
prompt; the prompt stays short on purpose and the rules live in the repository, where they can change without
